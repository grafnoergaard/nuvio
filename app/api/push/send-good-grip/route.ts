...
export async function POST(request: NextRequest) {
  ...
  const definition = getPushNotificationDefinition('good_grip');
  if (!definition) {
    return NextResponse.json({ error: 'Godt greb-push definition mangler' }, { status: 500 });
  }
  ...
  const [
    ...
    supabase
      ...
      .eq('notification_key', 'good_grip'),
  ]);

  if (subscriptionError || budgetError || expenseError || transitionError || flowConfigError || deliveryStateError) {
    return NextResponse.json({ error: 'Kunne ikke beregne godt greb-push' }, { status: 500 });
  }
  ...
  const deliveryStateByUser = new Map<string, { last_sent_week_key: string | null }>();
  for (const row of (deliveryStateRows ?? []) as Array<{ user_id: string; last_sent_week_key: string | null }>) {
    deliveryStateByUser.set(row.user_id, { last_sent_week_key: row.last_sent_week_key });
  }
  ...
  const candidates = ((budgetRows ?? []) as Array<{ user_id: string; budget_amount: number }>)
    ...
    .map((row) => {
      ...
      const remaining = monthlyBudget - monthlySpent;
      return { userId: row.user_id, score, remaining };
    })
    .filter((candidate) => candidate.score >= 72 && candidate.score < 85 && candidate.remaining >= 0)
    .filter((candidate) => deliveryStateByUser.get(candidate.userId)?.last_sent_week_key !== monthKey);
  ...
  if (!response.ok) {
    return NextResponse.json({ error: result?.error || 'Kunne ikke sende godt greb-push' }, { status: response.status });
  }
  ...
  await supabase
    ...
    .upsert(
      candidates.map((candidate) => ({
        notification_key: 'good_grip',
        user_id: candidate.userId,
        last_sent_at: now.toISOString(),
        last_sent_week_key: monthKey,
        last_sent_condition: 'grounded',
      })) as any,
