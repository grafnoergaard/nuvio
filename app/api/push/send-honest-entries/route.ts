...
export async function POST(request: NextRequest) {
  ...
  const definition = getPushNotificationDefinition('honest_entries');
  if (!definition) {
    return NextResponse.json({ error: 'Ærlige poster-push definition mangler' }, { status: 500 });
  }
  ...
  const [
    ...
    supabase
      ...
      .eq('key', 'honest_entries')
      .maybeSingle(),
    supabase
      .from('push_notification_user_state')
      .select('user_id,last_sent_at')
      .eq('notification_key', 'honest_entries'),
  ]);

  if (subscriptionError || budgetError || expenseError || deliveryStateError) {
    return NextResponse.json({ error: 'Kunne ikke beregne ærlige poster-push' }, { status: 500 });
  }
  ...
  const candidates = Array.from(uniqueBudgetUsers)
    ...
    .filter((userId) => isAssignedRandomDay(userId, 'honest_entries', now, timeZone))
    .filter((userId) => hasCooldownPassed(deliveryStateByUser.get(userId)?.last_sent_at ?? null, now, 18));
  ...
  if (!response.ok) {
    return NextResponse.json({ error: result?.error || 'Kunne ikke sende ærlige poster-push' }, { status: response.status });
  }
  ...
  await supabase
    ...
    .upsert(
      candidates.map((userId) => ({
        notification_key: 'honest_entries',
        user_id: userId,
        last_sent_at: now.toISOString(),
        last_sent_week_key: now.toISOString().slice(0, 10),
        last_sent_condition: 'random_foundation',
      })) as any,
