...
export async function POST(request: NextRequest) {
  ...
  const definition = getPushNotificationDefinition('score_drop');
  if (!definition) {
    return NextResponse.json({ error: 'Score-push definition mangler' }, { status: 500 });
  }
  ...
  const [
    ...
    supabase
      ...
      .eq('notification_key', 'score_drop'),
  ]);

  if (subscriptionError || budgetError || expenseError || transitionError || flowConfigError || deliveryStateError) {
    return NextResponse.json({ error: 'Kunne ikke beregne score-fald' }, { status: 500 });
  }
  ...
  const scoreThreshold = (flowConfigEntries ?? []).find((entry) => entry.key === 'NUVIO_FLOW_SCORE_PERFECT_THRESHOLD')?.value_numeric ?? 0.15;
  ...
  const candidates = ((budgetRows ?? []) as Array<{ user_id: string; budget_amount: number }>)
    ...
    .filter((candidate) => {
      ...
      if (!previous) return candidate.score < 60;
      if (previous.last_sent_week_key !== monthKey) return candidate.score < 60;

      const previousSeverity = previous.last_sent_condition === 'risk' ? 2 : previous.last_sent_condition === 'caution' ? 1 : 0;
      return candidate.severity > previousSeverity;
    });
  ...
  if (!response.ok) {
    return NextResponse.json({
      error: result?.error || 'Kunne ikke sende score-push',
  ...
  await supabase
    ...
    .upsert(
      candidates.map((candidate) => ({
        notification_key: 'score_drop',
        user_id: candidate.userId,
        last_sent_at: now.toISOString(),
        last_sent_week_key: monthKey,
        last_sent_condition: candidate.severity >= 2 ? 'risk' : 'caution',
      })) as any,
